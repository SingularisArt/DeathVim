let g:SingularisArtColorColumnBlacklist = ['diff', 'undotree', 'nerdtree', 'qf', 'NvimTree']
let g:SingularisArtCursorlineBlacklist = ['command-t']
let g:SingularisArtMkviewFiletypeBlacklist = ['diff', 'hgcommit', 'gitcommit']
...
function! autocmds#should_colorcolumn() abort
  return index(g:SingularisArtColorColumnBlacklist, &filetype) == -1
endfunction

function! autocmds#should_cursorline() abort
  return index(g:SingularisArtCursorlineBlacklist, &filetype) == -1
endfunction
...
function! autocmds#should_mkview() abort
  return
        \ &buftype == '' &&
        \ index(g:SingularisArtMkviewFiletypeBlacklist, &filetype) == -1 &&
