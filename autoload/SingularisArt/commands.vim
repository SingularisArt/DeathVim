...
function! s:open_on_github(file, range) abort
  let l:git_dir=SingularisArt#git#get_git_dir(a:file)
  if l:git_dir == ''
    call SingularisArt#functions#echoerr('No .git directory found above ' . a:file)
    return
  endif
...
function! SingularisArt#commands#find(args) abort
    set errorformat+=%f
...
function! SingularisArt#commands#glow(file) abort
  if !executable('glow')
...
function! SingularisArt#commands#lint() abort
...
function! SingularisArt#commands#marked(file) abort
  if empty(a:file)
...
function! SingularisArt#commands#open_on_github(...) abort range
  let l:range=''
  if a:0 == 0
    let l:files=[expand('%')]
  ...
  if !l:did_open
    call SingularisArt#functions#echoerr('No filename')
  endif
endfunction

function! SingularisArt#commands#preview(file) abort
  if executable('open')
    call SingularisArt#commands#marked(a:file)
  elseif executable('glow')
    call SingularisArt#commands#glow(a:file)
  else
    echoerr 'No "open" or "glow" executable found'
  endif
endfunction

function! SingularisArt#commands#typecheck() abort
  " Make subsequent `:make` work (eg. invoked by Dispatch's `m<CR>` mapping).
  compiler tsc
...
function! SingularisArt#commands#vim() abort
