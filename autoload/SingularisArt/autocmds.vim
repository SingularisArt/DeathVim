function! SingularisArt#autocmds#attempt_select_last_file() abort
...
let s:encrypted[expand('~/SingularisArt')]='third-party-tools/git-cipher/bin/git-cipher'

" Update encryptable files after saving.
function! SingularisArt#autocmds#encrypt(file) abort
...
let g:SingularisArtEditorConfigPathBlacklist=[]

let g:SingularisArtEditorConfigFileTypeBlacklist=[
      \   'NvimTree',
      \   'command-t',
      \   'diff',
      \   'dirvish',
...
function! s:editorconfig_blacklist(path, filetype)
  for l:item in g:SingularisArtEditorConfigPathBlacklist
    if match(a:path, l:item) != -1
      return 1
    endif
  endfor

  for l:item in g:SingularisArtEditorConfigFileTypeBlacklist
...
function! SingularisArt#autocmds#apply_overrides() abort
  let l:path=expand('%:p')
  if empty(l:path)
    return
  endif
  ...
  let l:editorconfig=SingularisArt#autocmds#editorconfig(l:path)
        ...
        if l:key == 'indent_style'
          if l:value == 'space'
            ...
            if match(&formatprg, '^par ') != -1
              ...
              call operator#user#define('format-and-retab', 'SingularisArt#autocmds#format')
            endif
...
function! SingularisArt#autocmds#editorconfig(file) abort
  " Make absolute path.
  ...
  if l:root == 0 && l:path != '' && l:path != '/'
    ...
    return extend(SingularisArt#autocmds#editorconfig(l:parent), l:config)
  else
    return l:config
  end
endfunction

function! SingularisArt#autocmds#format(motion) abort
